package com.hqup.gamexo.ai.gardnerway;

import java.util.ArrayList;

import com.hqup.gamexo.ai.IBrainAI;
import com.hqup.gamexo.ai.utils.FieldMatrixConverter;
import com.hqup.gamexo.ai.utils.LoggerAI;

/**
 * Date: 03.09.13 Time: 15:33 Вычисления ходов данного ИИ основываются на
 * принципе коробочных самообучающихся машин. Обучение ИИ состоит в том, что
 * проигрышные позиции запоминаются и при следующей партии ходы, приводящие к
 * тем же позициям, блокируются. Таким образом накапливается опыт ИИ и сила его
 * игры постепенно возрастает. Проигрышные позиции хранятся в файлах, и
 * извлекются на момент проверки. Для уменьшения количества записанных позиций,
...
public class Gardner implements IBrainAI<Object> {
	 ...
	 * вычисления следующего хода. Перегрузка методов получилась из желания как
	 * можно меньше переделывать код, и иметь более гибкий интерфейс для
	 * тестирования.
	 * 
	 * @param fieldMatrixObject
	 *            матрица поля, переданная ядром.
	...
	/**
	 * проверяет в соответствии с входящими координатами возможность выигрыша
	 * противником, и делает простой рандомный ход. Если противник может
	 * выиграть следующим ходом, то он занимает победное поле своей фишкой.
	 * 
	 * @param enemyX
	 *            координата по X
	 * @param enemyY
	...
	public int[] findMove(int enemyX, int enemyY) {
		...
		while (true) {
			...
			start: while (true) {
				/*
				 * Мозг червяка - если ИИ видит, что следующим ходом противник
				 * выиграет, то ставит в это место свою фишку.
				 */
				if (move[X] == 25 || move[Y] == 25) {
	...
	/**
	 * Сверяет новую матрицу доски с предыдущей матрицей и определяет координаты
	 * последнего хода. Так же ИИ вычисляет очередь своих ходов и оперирует
	 * своим внутренним порядком фишек в зависимости от полученного результата.
	 * 
	 * @param fieldMatrix
	 *            поле, которое пришло из ядра
